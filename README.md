# Touris 知途 · 记忆驱动 + 算法排线的旅行规划

以「记忆」为护城河的旅行规划产品原型。v2 起补齐了三块拼图：

1. **路线不再是预制线路**——每城 50+ 候选景点，路线由「LLM 提名候选 → 算法排线」实时算出；
2. **RAG 攻略检索**——按目的地 / 出发季节 / 天数三道硬约束查相似攻略，按你的偏好与强度改编；
3. **交互正常化**——预设身份下拉（无注册无密码）、顶栏返回键与面包屑、浏览器前进后退可用、切屏滚动归零。
...
需求（目的地 / 日期 / 天数 / 偏好 / 强度）
   ...
   │     从 50+ 候选池按记忆偏好提名约 days×5 个；名字必须逐字命中候选池，
   │     越界提名整条丢掉（闸门）。没后端 → 本地评分，流程不断。
   │
...
**「默认版 vs 记忆版」= 引擎跑两次**（0 记忆 vs 全部记忆），diff 由两次结果
逐天对齐得出——六座城市走同一套推导，不再有任何手工对照。
...
- `constraintsOf()` 聚合所有记忆 → `avoid` 直接出局、`prefer` 加分、`pace` 决定
  每天点位上限与出发时间（记忆 pace 优先于用户选的强度——事实压过意愿）；
- 每处取舍都通过 `whoContributes()` 反查到具体记忆 id，界面上的 🧠 可点开源。

三份预置档案给出三种完全不同的推荐：**林小满**（慢节奏·晕博物馆·爱市集）、
**陈铁腿**（特种兵·博物馆控·要夜景）、**周晚晚**（自然派·怕人多·日落刚需）。

## 目录结构

| 路径 | 说明 |
| --- | --- |
| `prototype/` | **原型本体**（纯静态前端，所有改动只改这里） |
| ├ `semantics.js` | 语义词表 + 景点中文标签映射（记忆 → 推荐的唯一通道） |
| ├ `data.js` | 反馈规则表 + 三份预置身份档案 |
| ├ `city-data.js` / `city-expansion.js` / `city-hangzhou-guangzhou.js` / `city-chengdu.js` | 城市资料（餐饮候选、住宿、攻略来源、权威坐标） |
| ├ `spots-expansion.js` | **候选景点扩充包**：每城补到 50+（坐标/游玩时长/标签），并做归一化（poi 由 geo 反算、餐饮锚点、center） |
...
```bash
# 方式一：纯静态（无 LLM，前端自动降级为本地算法）
npm run serve            # http://localhost:8000

# 方式二：带 LLM 后端（零依赖，Node 18+）
...
**没配 Key 一切照常**：候选提名退回本地评分、攻略退回本地模板，
生成流水线第 2 步会如实标注「未接入大模型服务」——路演当天服务挂了不开天窗。
...
模型只**提名**景点、只**改编**检索到的攻略；路线排序、时间表、diff
全部由 `planner.js` 的确定性算法产出——「每条变更都能反查到哪条记忆」不断。
...
```bash
npm verify     # 候选池完整性 · 规划器 648 种组合 · RAG 硬约束 ·
               # 身份播种 · LLM 降级路径 · 桩 DOM 实跑 28 种渲染组合
npm run imgcheck   # 图片引用核对
```

## LOGO

原始素材 `TOURIS.png`（知途 + TOURIS 中英组合）已遗失，但由它生成的三个位图
变体完整保留在 `prototype/logo.css`（`--logo-mark` / `--logo-word` / `--logo-lock`，
data URI 内联、离线可用）。v2 起顶栏与首屏恢复使用完整中英组合 `--logo-lock`。
