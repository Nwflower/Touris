...
杭州 18 个、广州 21 个候选景点，每城三种主题，支持 **2—7 天**。从首页或需求表单选择几天，就生成对应天数；详情和记忆对照保持一致。新增真实 OpenStreetMap 底图、景点标记、拖动缩放和来源入口，虚线表示游览顺序，不是导航路线。

保留原有照片并补入 27 张有作者及许可记录的图片。资料与维护说明见 [城市素材文档](docs/cities/README.md)，分天攻略见 [多日路线](docs/cities/multi-day-guides.md)。

回归验证：`cd prototype && node _verify.js`。发布前运行 `./sync-pages.sh`。
