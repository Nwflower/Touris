    ...
    const days = wants.map(w => {
      ...
      const entries = picks.map(p => {
        return {
          ...
          time: (d.times || [])[p.slot] || '',
          /* 补进来的点**不沿用槽位原有的时长与备注**。
             一开始我把槽位时长当成"时间预算"留给替补，实测出了反例：北京 D2 原本是
             「一整天去慕田峪长城」（槽位 300 min），长城被砍后补进亮马河，页面于是
             显示「亮马河 · 300 min」——一条河边步道被标成 5 小时。槽位时长是**那个点**
             的停留时长，不是一段可挪用的预算。备注同理（那是写给原点的）。
             时刻保留：它是"当天第几站"，与具体景点无关。 */
          dur:  p.origin === 'added' ? '' : ((d.durs || [])[p.slot] || ''),
